...
public class Blackjack {
	...
	/**
	 * Metodo que da la bienvenida al jugador y lo saluda por su nombre
     */
    //@ ensures \result != null;
     ...
     */
	//@ requires 1 <= puntosCrupier <= 28;
	//@ requires 2 <= puntosJugador <= 31;
	public static void mostrarPuntuacionesPorTexto(int puntosCrupier, int puntosJugador, boolean mostrarCartasCrupier,
            Carta[] crupier) {
		System.out.println("");
	...
	public static void mostrarCreditosPorTexto(int credito, int apuesta) {
	...
	//@ requires 1 <= valorManoCrupier <= 28;
	//@ requires 2 <= valorManoJugador <= 31;
	//@ requires crupier.length == 17;
	...
	public static /*@ pure @*/ void mostrarCreditos(MaquinaDeTrazados mesa, int anchoMesa, int alturaMesa,
			int credito, int apuesta) {
		// Convertimos credito y apuesta de int a String para poder escribirlas
		// en la interfaz grafica
		...
		// Finalmente mostramos el numero de creditos disponibles y la cantidad
		// de creditos apostados en la mano
	...
	/**
	 * Metodo con el que se reparten las 56 posibles cartas en un mazo
	 * para que sea posible sacar cualquiera de ellas al azar posteriormente
     */
	//@ ensures \result != null;
	...
	/**
	 * Metodo con el que se dibujan todas las cartas de la mano del
	 * crupier en la interfaz grafica. Mientras no termine la mano, solo
	 * se dibuja la primera carta del crupier visible y el resto volteadas.
	 * Luego se dibujan todas las cartas de forma visible cuando termina la
	 * mano y se da el resultado
     */
	//@ requires mesa != null;
	//@ requires 0 < mesa.XMAX < Integer.MAX_VALUE;
	//@ requires 0 < mesa.YMAX < Integer.MAX_VALUE;
	...
	//@ requires 30 <= alturaCarta <= 10000;
	//@ requires 30 <= anchoCarta <= 10000;
	//@ requires sizes.length == 2 || sizes.length == 3;
	//@ requires 2 <= numCartasCrupier < 17;
	//@ requires crupier.length == 17;
	public static void dibujarCartasCrupier(MaquinaDeTrazados mesa, int anchoMesa, int anchoCarta, int alturaCarta, int[] sizes,
		...
		int i = 0;
		int j = 0;
		//@ maintaining 0 <= i <= numCartasCrupier;
	...
	/**
	 * Metodo con el que se dibujan todas las cartas de la mano del
	 * jugador en la interfaz grafica. Todas las cartas del jugador se
	 * dibujar de forma visible
     */
	//@ requires mesa != null;
	//@ requires 0 < mesa.XMAX < Integer.MAX_VALUE;
	//@ requires 0 < mesa.YMAX < Integer.MAX_VALUE;
	...
	//@ requires 30 <= anchoCarta <= 10000;
	//@ requires sizes.length == 2 || sizes.length == 3;
	//@ requires 2 <= numCartasJugador < 21;
	//@ requires jugador.length == 21;
	...
	/**
	 * Metodo con el que se despide al jugador llamandolo por su nombre
	 * y se le agradece por haber jugado
     */
	//@ requires nombre != null;
    public static void mensajeDespedida(String nombre) {
        System.out.println("");
    ...
    public static void main(String[] args) {
        ...
        boolean seJugaraOtraMano, mostrarCartasCrupier, finalizoLaMano, mostrarResultado, hayBlackjack, sePasaronDeVeintiuno, resultadoMostrado, calcularResultadoMano;
        seJugaraOtraMano = mostrarCartasCrupier = finalizoLaMano = mostrarResultado = resultadoMostrado = hayBlackjack = sePasaronDeVeintiuno = calcularResultadoMano = false;
		...
		while (credito >= 0 && numManosJugadas < 5) {
            ...
            if (sePasaronDeVeintiuno == false && hayBlackjack == false && resultadoMostrado == false) {
                ...
                if (decisionJugador == 1) {
                    ...
                    mostrarCartasCrupier = finalizoLaMano = mostrarResultado = calcularResultadoMano = true;
			...
			if (finalizoLaMano == true && resultadoMostrado == true) {
                ...
                if (credito < 10) {
                    System.out.println("");
                    System.out.println("Tienes menos creditos que la apuesta minina para una mano. Se termina el juego.");
                    break;
